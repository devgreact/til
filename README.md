...
## 3. npm 버전 확인하기 (Node Package Mange)
...
- 원하는 소스를 npm.com 에서 다운로드 받아서 설치한다.(추후)
- 수작업으로 기본 Node.js 프로젝트 를 생성해 봅시다.
...
// common.js 에서 외부에 함수를 오픈시킴
module.exports = {
  say, // 객체 속성 축약형(추천)
  hohoho: hi, // 개체 이름 변경 (아주 드문 케이스)
  smile: smile, // 객체 이름과 값이름 같게 (아주 드문 케이스)
};
```
...
- 꼭 체크하자. `프로젝트 폴더 구조 변화` 및 `packge.json 변화`
- 모듈 소스 관리는 `npm` 또는 `yarn` 으로 합니다.
- `npm -v` 설치를 확인함.

### 6.1. `npmjs.com` 에서 모듈 선택시 고려사항

- `TS` 지원되는지(TypeScript 지원안되면 고민해보자.)
- 다운로드 수 (10만 이하는 고민해보자.)
- 주기적 관리 (version 관리, 2년이상 관리안되면 고민해보자.)
...
- 모듈 초기 셋팅 : package-lock.json 제거, node_modules 폴더제거
